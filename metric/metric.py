...
class Metrics():
    ...
    def __get_intersect_by_entity__(self, pred_class_span, label_class_span):
        ...
        return cnt
    def __get_intersect_by_entity_label__(self, pred_class_span, label_class_span):
        '''
        return the count of correct entity
        '''
        label_dict={}
        for label in label_class_span:
            if label not in label_dict:
                label_dict[label] = 0
            label_dict[label] += len(list(set(label_class_span[label]).intersection(set(pred_class_span.get(label,[])))))
        return label_dict
    ...
    def __get_cnt__(self, label_class_span):
        ...
        return cnt
    def __get_cnt_by_label__(self, label_class_span):
        '''
        return the count of entities
        '''
        label_dict={}
        ...
        for label in label_class_span:
            if label not in label_dict:
                label_dict[label]=0
            label_dict[label] += len(label_class_span[label])
        return label_dict
    ...
    def metrics_by_entity_(self, pred, label):
        ...
        pred_dict_cnt = self.__get_cnt_by_label__(pred_class_span)
        label_dict_cnt = self.__get_cnt_by_label__(label_class_span)
        correct_dict_cnt = self.__get_intersect_by_entity_label__(pred_class_span,label_class_span)
        correct_cnt = self.__get_intersect_by_entity__(pred_class_span, label_class_span)
        return pred_cnt, label_cnt, correct_cnt ,pred_dict_cnt,label_dict_cnt,correct_dict_cnt
