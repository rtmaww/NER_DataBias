...
from data_process.data_utils import convert_examples_to_features, get_labels, read_examples_from_file,InputExample

from transformers import AdamW, get_linear_schedule_with_warmup
from transformers import WEIGHTS_NAME, AutoModel,AutoConfig,AutoTokenizer
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.max_steps > 0 and global_step > args.max_steps:
                ...
                break
        metric_results = cal_metric(preds=preds,out_label_ids=out_label_ids,labels=labels,pad_token_label_id=pad_token_label_id)
        logger.info(f"***** train results epoch {epoch_count} *****")
        for key in sorted(metric_results.keys()):
            logger.info("  %s = %s", key, str(metric_results[key]))
...
def fast_evaluate(args, ckpt_dir, config, tokenizer, labels,
    ...
    result = {"{}_{}".format(prefix, k): v for k, v in result.items()}
    logger.info(result)
    return result
...
def get_entity_candidates(data_dir,name):
    with open(f'{data_dir}/{name}.json') as f:
        content = json.load(f)
    return content

def get_replace_entity_pos(label,label_mask):
    pos_list=[]
    for ind,(tag,mask) in enumerate(zip(label,label_mask)):
        if tag .startswith("B-") and mask == 0:
            pos_start = ind
            pos_end = ind+1
            while pos_end < len(label) and label[pos_end].startswith("I-"):
                pos_end +=1
            pos_list.append([pos_start,pos_end])
    return pos_list
...
def main():
    ...
    parser.add_argument("--output_dir", default="ontonotes/model/lr_0.00005_bsz_16/random_-1_1", type=str)
    parser.add_argument("--train_mode", default="random_1000_1_train", type=str)
    ## Other parameters
    ...
    if args.do_predict:
        ...
        # Save results
        output_test_results_file = os.path.join(best_checkpoint, "new_test_results.txt")
